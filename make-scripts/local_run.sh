...
docker run -it --rm --name repast-local-docker -e PYTHONFAULTHANDLER=1 -v "$PWD":/usr/src/myapp -w /usr/src/myapp repast-local-docker mpirun -n 6 python ./repast4py/deer_model.py ./config/local_deer_config.yaml
